...
// Target the income tab rendering section
const targetStart = "tabName === 'income'";
const startIdx = html.indexOf(targetStart);

if (startIdx !== -1) {
    // Find the end of this case/if block (up to next tab condition)
    let endIdx = html.indexOf('} else if', startIdx);
    if (endIdx === -1) endIdx = html.indexOf('} else', startIdx);

    const newIncomeBlock = `tabName === 'income' {
        ...
        currentUser.rigs.forEach((r, index) => {
            const rentedTime = new Date(r.rentedAt || Date.now()).getTime();
            const now = new Date().getTime();
            
            // Calculate active days with 6:00 AM daily anchor
            let daysActive = 0;
            ...
            htmlContent += \`
                <div style="background:#0b1329; border:1px solid #1e2952; padding:14px; border-radius:8px; margin-top:12px;">
                    <h4 style="color:#38bdf8; font-size:15px; margin-bottom:4px;">\${r.name}</h4>
                    <p style="font-size:12px; color:#cbd5e1; margin-top:2px;">Purchase Price: <span style="color:#f87171; font-weight:bold;">UGX \${(r.price || 0).toLocaleString()}</span></p>
    ...
    html = html.substring(0, startIdx) + newIncomeBlock + html.substring(endIdx);
    fs.writeFileSync('index.html', html);
    console.log("Income tab rendering updated successfully!");
} else {
    console.log("Could not find income tab block.");
}
