...
const target = "if (tabName === 'income')";
const startIdx = html.indexOf(target);

if (startIdx !== -1) {
    // Find where this income if block ends (before next 'else if' or closing switchTab block)
    let endIdx = html.indexOf('} else if', startIdx);
    if (endIdx === -1) endIdx = html.indexOf('} \n    }', startIdx);
    if (endIdx === -1) endIdx = html.indexOf('} else {', startIdx);

    const completeIncomeCode = `if (tabName === 'income') {
    ...
    html = html.substring(0, startIdx) + completeIncomeCode + html.substring(endIdx);
    fs.writeFileSync('index.html', html);
    console.log("Income block inside switchTab successfully patched!");
} else {
    console.log("Could not find income block in switchTab.");
}
