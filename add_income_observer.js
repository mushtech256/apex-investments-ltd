...
// Let's add a clean income observer script right before the closing script tag
const observerScript = `
    // Income Tab Dynamic Observer
    setInterval(() => {
        const title = document.getElementById('header-title');
        const container = document.getElementById('main-container');
        if (title && title.innerText === 'Income Overview' && container) {
            // Check if we already rendered active rigs or if it shows "No Active Machines"
            if (!container.innerHTML.includes('Active Rigs & Progress') && !container.innerHTML.includes('No Active Machines')) {
                renderIncomeTab(container, title);
            }
        }
    }, 500);

    function renderIncomeTab(container, title) {
...
// Replace closing script tag with our observer and claim functions
if (!html.includes('renderIncomeTab')) {
    html = html.replace('</script>', observerScript);
    fs.writeFileSync('index.html', html);
    console.log("Income observer injected successfully!");
}
