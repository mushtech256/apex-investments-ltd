...
// Replace the income rendering block inside switchTab to fetch fresh user data
const target = "else if (tabName === 'income') {";
const idx = html.indexOf(target);

if (idx !== -1) {
    let endIdx = html.indexOf('} else if', idx);
    if (endIdx === -1) endIdx = html.indexOf('} \n    }', idx);
    if (endIdx === -1) endIdx = html.indexOf('} else {', idx);

    const liveIncomeCode = `else if (tabName === 'income') {
        title.innerText = 'Income Overview';
        container.innerHTML = \`<div class="card" style="text-align:center; padding: 20px;"><p style="color:#cbd5e1;">Loading active rigs...</p></div>\`;
        
        const phone = currentUser?.phone || currentUser?.phone_number || localStorage.getItem('userPhone');
        
        fetch('/api/user/profile?phone=' + encodeURIComponent(phone))
            .then(res => res.json())
            .then(data => {
                const rigs = data.user?.rigs || currentUser?.rigs || [];
                if (rigs.length === 0) {
            ...
            .catch(err => {
                console.error(err);
                container.innerHTML = \`<div class="card" style="text-align:center; padding:20px;"><p style="color:#ef4444;">Failed to load rigs data.</p></div>\`;
            });
    }`;

    html = html.substring(0, idx) + liveIncomeCode + html.substring(endIdx);
    fs.writeFileSync('index.html', html);
    console.log("Live fetch income patch applied!");
}
